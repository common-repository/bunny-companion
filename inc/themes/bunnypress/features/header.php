...
$selective_refresh = isset( $wp_customize->selective_refresh ) ? 'postMessage' : 'refresh';
	...
	=========================================*/
	$wp_customize->add_setting(
		FASTERLY_THEME_SETTINGS.'_hdr_left_txt_head'
			,array(
			'capability'     	=> 'edit_theme_options',
			'sanitize_callback' => 'fasterly_sanitize_text',
		)
	...
	FASTERLY_THEME_SETTINGS.'_hdr_left_txt_head',
		array(
			'type' => 'hidden',
			'label' => __('Animate Text','bunny-companion'),
			'section' => 'top_header',
			'priority' => 1,
		)
	);
	...
	$wp_customize->add_setting( 
		FASTERLY_THEME_SETTINGS.'_hs_hdr_left_txt' , 
			array(
			'default' => '1',
			'capability'     => 'edit_theme_options',
	...
	// Title // 
	$wp_customize->add_setting(
    	FASTERLY_THEME_SETTINGS.'_hdr_left_ttl',
    	array(
	...
	// Content // 
	$wp_customize->add_setting(
    	FASTERLY_THEME_SETTINGS.'_hdr_left_txt',
    	array(
	        'default'			=> __('"You Search Business plans for us?","You Search Business plans for us?","You Search Business plans for us?"','bunny-companion'),
	...
	// Title // 
	$wp_customize->add_setting(
    	FASTERLY_THEME_SETTINGS.'_hdr_social_ttl',
    	array(
	        'default'			=> __('Follow Us On','bunny-companion'),
			'sanitize_callback' => 'fasterly_sanitize_html',
			'transport'         => $selective_refresh,
			'capability' => 'edit_theme_options',
		)
	...
	=========================================*/
	$wp_customize->add_setting(
		FASTERLY_THEME_SETTINGS.'_hdr_contact_head'
			,array(
			'capability'     	=> 'edit_theme_options',
			'sanitize_callback' => 'fasterly_sanitize_text',
			'priority' => 3,
	...
	FASTERLY_THEME_SETTINGS.'_hdr_contact_head',
		array(
			'type' => 'hidden',
			'label' => __('Contact Info','bunny-companion'),
			'section' => 'top_header',
		)
	);
	...
	// Hide / Show
	$wp_customize->add_setting( 
		FASTERLY_THEME_SETTINGS.'_hs_hdr_contact' , 
			array(
			'default' => '1',
			'capability'     => 'edit_theme_options',
			'sanitize_callback' => 'fasterly_sanitize_checkbox',
	...
	// Contact Info
	$wp_customize->add_setting( FASTERLY_THEME_SETTINGS.'_hdr_ct_info', 
			array(
			 'sanitize_callback' => 'fasterly_repeater_sanitize',
			 'priority' => 8,
			 'default' => fasterly_hdr_contact_info_default()
			)
		);
		
		$wp_customize->add_control( 
			new Fasterly_Repeater( $wp_customize, 
				FASTERLY_THEME_SETTINGS.'_hdr_ct_info', 
					array(
						'label'   => esc_html__('Contact Info','bunny-companion'),
						'section' => 'top_header',
						'add_field_label'                   => esc_html__( 'Add New Contact', 'bunny-companion' ),
						'item_name'                         => esc_html__( 'Contact', 'bunny-companion' ),
						'customizer_repeater_icon_control' => true,
	...
	//Premium feature
		class fasterly_theme_contact_premium extends WP_Customize_Control {
			public function render_content() { 
			?>
				<a class="customizer_fasterly_contact_premium up-to-pro" href="<?php echo esc_url(fasterly_pro_links()); ?>" target="_blank" style="display: none;"><?php _e('More Contact Available in Premium Version','bunny-companion'); ?></a>
			<?php
			} 
		}	
	
		$wp_customize->add_setting( 'fasterly_contact_pro', array(
			'capability'			=> 'edit_theme_options',
			'sanitize_callback'	=> 'wp_filter_nohtml_kses',
			'priority' => 8,
		));
		$wp_customize->add_control(
			new fasterly_theme_contact_premium(
			$wp_customize,
			'fasterly_contact_pro',
				array(
					'section'				=> 'top_header',
				)
			)
